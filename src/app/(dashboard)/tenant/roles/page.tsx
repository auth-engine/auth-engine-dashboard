...
export default function TenantRolesPage() {
    ...
    const { activeTenantId, isReady, isPlatformTenant, isLoading: isLoadingTenant } = useActiveTenant();
    ...
    const saveMutation = useMutation({
        mutationFn: async (payload: RolePayload) => {
            ...
            const { data } = await apiClient.post(`/tenants/${activeTenantId}/roles`, payload);
            return data;
        },
        onSuccess: () => {
            toast.success(editingRole ? "Role updated!" : "Role created!");
            queryClient.invalidateQueries({ queryKey: ["tenantRoles", activeTenantId] });
            ...
            <Dialog open={isDeleting} onOpenChange={setIsDeleting}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to delete <strong>{deletingRole?.name}</strong>? Users assigned to this role will lose these permissions.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
